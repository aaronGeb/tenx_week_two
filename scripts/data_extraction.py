...
class DataExtractor:
    ...
    def read_csv_file(file_path):
        """
        Read a CSV file, extract columns and data.
        ...
        - data (list): List of lists representing the data.
        """
    ...
    def prepare_data_frame(self, trajectory_data: tuple, timed_vehicle_data: tuple):
        ...
        try:
            ...
            trajectory_data = pd.DataFrame(
                columns=trajectory_cols, data=trajectory_rows
            )
            timed_vehicle_data = pd.DataFrame(
                columns=timed_vehicle_cols, data=timed_vehicle_rows
            )
    ...
    def extract_data(self, file_name: str, return_json=False) -> pd.DataFrame:
        try:
            ...
            trajectory_data, timed_vehicle_data = self.prepare_data_for_pandas(
                columns=columns, all_data=all_data, id_prefix=id_prefix
            )
            if not return_json:
